...
const generateToken = (user) => {
  return jwt.sign(
    { 
      id: user._id, 
      mail: user.mail,
      role: user.role   // ← CORRECTION PRINCIPALE : on ajoute le rôle ici !
    },
  ...
  exports.login = async (req, res) => {
    try {
      ...
      // Recherche secondaire selon le rôle
      let profileId = null;

      if (user.role === 'boutique') {
        const boutique = await Boutique.findOne({ mail: user.mail });
        profileId = boutique ? boutique._id : null;
      ...
      const token = jwt.sign(
        { 
          ...
          mail: user.mail, 
          role: user.role,
          profileId  // idBoutique ou idClient selon le rôle, null si supermarche
        },
      ...
      res.json({
        success: true,
        token,
        user: { id: user._id, nom: user.nom, mail: user.mail, role: user.role, profileId }
      });
